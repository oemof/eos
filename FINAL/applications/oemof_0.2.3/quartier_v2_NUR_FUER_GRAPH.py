...
def create_energysystem(energysystem, parameters,
    ...
    energysystem.add(solph.components.GenericStorage(
        ...
        investment=solph.Investment(ep_costs=parameters['storage_epc']+parameters['opex_bat'])))
    ...
    if arguments['--ssr']:
        ...
        energysystem.add(solph.Source(label='gridsource', outputs={
    ...
    for house in parameters['hh']:
        ...
        energysystem.add(bel_pv)

        # Create excess component for bel_pv to allow overproduction
        energysystem.add(solph.Sink(label=house+"_excess", inputs={bel_pv: solph.Flow()}))

        # Create sink component for the pv feedin
        if arguments['--feedin']:
            energysystem.add(solph.Sink(label=house+'_feedin', inputs={bel_pv: solph.Flow(
        ...
        if arguments['--pv-costopt']:
            energysystem.add(solph.Source(label=house+'_pv', outputs={bel_pv: solph.Flow(
...
def main(**arguments):
    ...
    energysystem_graph = graph.create_nx_graph(esys, filename='graph.graphml')
    ...
    graph_file_name = 'energysystem_graph.graphml'
